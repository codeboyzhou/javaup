...
一台开发机上往往同时存在多个 Java 项目。切换项目时，开发者经常需要修改环境变量、
...
- **Maven Toolchains** 允许 Maven 插件选择 JDK；`jup` 还会读取 `<jdkHome>`，
  并控制启动 Maven 自身所用的 JDK。
...
请先阅读[安装](getting-started/installation/)和[快速开始](getting-started/quick-start/)，
也可以直接查看[命令参考](reference/command-reference/)。
