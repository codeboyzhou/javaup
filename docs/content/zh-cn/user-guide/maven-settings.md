...
命名别名适合在公司私服、公共镜像和带有不同凭据的环境之间切换，无需替换全局文件。
...
`jup` 会检查路径是否为普通文件、XML 是否有效，以及根元素是否为 `<settings>`。
配置只保存规范化后的路径，不会复制文件内容或凭据。
...
绑定保存在本地项目配置中。启动 Maven 时，`jup` 会自动添加对应的 `--settings`
参数。
...
`unset` 只修改当前项目；`remove` 删除全局别名。如果仍有项目引用已删除的别名，
请先绑定其他别名或执行 `jup settings unset`，再运行构建。
