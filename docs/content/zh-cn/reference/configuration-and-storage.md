...
安装或运行 `jup` 前，可以将 `JAVAUP_HOME` 设置为其他绝对路径。项目配置保存的是绝对路径快照；移动项目、JDK、Maven Wrapper 或 Maven 安装后，请重新执行 `jup init`。

settings 别名只保存路径，不保存 XML 内容或凭据。请按照引用文件自身的安全要求管理这些文件。
