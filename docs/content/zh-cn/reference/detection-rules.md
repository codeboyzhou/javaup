...
`jup init` 使用当前目录，并要求其中存在 `pom.xml`。初始化后，项目级命令可以从
任意子目录向上查找项目。项目路径、符号链接以及 Windows 长路径/8.3 短路径会被
规范化，避免同一个项目产生多份配置。

## Maven 选择

如果项目包含 `mvnw` 或 `mvnw.cmd`，`jup` 会保存 Wrapper；否则从 PATH 查找
`mvn` 或 `mvn.cmd`。最终解析的可执行文件和 Maven 版本会写入项目配置。
...
候选必须包含 `bin/javac`（Windows 为 `bin/javac.exe`）。版本优先从 JDK 的
