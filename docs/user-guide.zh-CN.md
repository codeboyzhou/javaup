...
- 保存的项目配置能否读取；
- `pom.xml` 能否解析，以及能否确定项目要求的 Java 版本；
- 保存的 Maven 或 Maven Wrapper 是否仍然存在并可执行；
- 保存的 JDK 是否包含 `javac`、能否识别版本，以及是否满足 POM；
- 绑定的 Maven settings 别名能否解析到有效的 `<settings>` 文件。

每项结果标记为 `PASS`、`WARN` 或 `FAIL`。POM 未声明 Java 版本时给出警告，因为仍可
使用保存的 JDK；资源损坏时给出失败结果，并附带恢复资源或重新执行 `jup init` 等
修复建议。
